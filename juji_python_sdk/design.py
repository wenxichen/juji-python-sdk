...
def set_browser_key(engagement_id: str | uuid.UUID, browser_key: str | uuid.UUID | None = None):
    """
    ...
    response = requests.post(
        f"{JUJI_PLATFORM_URL}/api/graphql",
        json={
            "query": mutation,
            "variables": variables
        },
...
def add_faq(questions: list[str], answers: list[str], engagement_id: str | uuid.UUID, browser_key: str | uuid.UUID | None = None):
    """
    ...
    # set browser key
    set_browser_key_response = set_browser_key(engagement_id, browser_key)

    if not set_browser_key_response["success"]:
        raise Exception(set_browser_key_response["message"])
    
    # get brand
    brand_name = get_brands()[0]["name"]
